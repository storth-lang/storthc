...
ST_string_t ST_token_kind_to_string(ST_token_kind_t kind) {
    ...
    switch (kind) {
    case ST_TINT:
        return ST_cstr_to_str("ST_TINT");
    case ST_TFLOAT:
        return ST_cstr_to_str("ST_TFLOAT");
    case ST_TSTRING:
        return ST_cstr_to_str("ST_TSTRING");
    case ST_TCHAR:
        return ST_cstr_to_str("ST_TCHAR");
    case ST_TIDENT:
        return ST_cstr_to_str("ST_TIDENT");
    case ST_TTYPE:
        return ST_cstr_to_str("ST_TTYPE");
    case ST_TKEYWORD:
        return ST_cstr_to_str("ST_TKEYWORD");
    case ST_TSYMBOL:
        return ST_cstr_to_str("ST_TSYMBOL");
    case ST_TDOCCOMENT:
        return ST_cstr_to_str("ST_TDOCCOMENT");
    case ST_TCOUNT:
    default:
        ST_assert(0);
    }
...
static b8 ST_isdigit(char c) { return c >= '0' && c <= '9'; }
...
static b8 ST_isident(char c) { return ST_isalpha(c) || ST_isdigit(c); }
...
b8 ST_is_keyword(ST_string_t s) {
    ST_forrange(0, ST_KEYWORD_COUNT) if (ST_string_eq_cstr(
                                             s, ST_keyword_names[i])) return 1;
    return 0;
}

b8 ST_is_primitive(ST_string_t s) {
    ST_forrange(
        0, ST_TYPE_COUNT) if (ST_string_eq_cstr(s, ST_type_names[i])) return 1;
    return 0;
}

b8 ST_is_symbol(ST_string_t s) {
    if (s.len == 1) {
        ST_forrange(0, sizeof(ST_single_symbols) -
                           1) if ((char)s.data[0] ==
                                  ST_single_symbols[i]) return 1;
        return 0;
    }
    ST_forrange(0, ST_array_len(ST_multi_symbols)) if (ST_string_eq_cstr(
                                                           s,
                                                           ST_multi_symbols
                                                               [i])) return 1;
    return 0;
...
void ST_colorize_source_line(FILE *out, ST_string_t text, const char *code_col,
                             const char *com_col) {
    u32 split = text.len;
    ...
    if (split < text.len)
        fprintf(out, "%s%.*s", com_col, (int)(text.len - split),
                (char *)text.data + split);
    fprintf(out, ST_COLOR_RESET);
}

static void ST_lx_error(ST_lexer_t *l, u32 line, u32 col, u32 pos,
                        const char *msg) {
    l->failed = 1;
    fprintf(stderr,
...
void ST_token_error(ST_string_t src) { ST_unused(src); }
...
static b8 ST_lx_escape(ST_lexer_t *l, ST_sb_t *sb, u32 line, u32 col, u32 pos) {
    ...
    case 't':
        ...
        return 1;
    case 'r':
        ST_sb_char_push(sb, '\r');
        return 1;
    case '0':
        ST_sb_char_push(sb, '\0');
        return 1;
    case 'e':
        ST_sb_char_push(sb, '\x1b');
        return 1;
    case '\\':
        ST_sb_char_push(sb, '\\');
        return 1;
    ...
    case 'x': {
        ...
        if (!ST_ishex(h1) || !ST_ishex(h2)) {
            ST_lx_error(l, line, col, pos,
                        "invalid hex escape, expected \\xNN");
            return 0;
        }
        ST_lx_advance_char(l);
        ST_lx_advance_char(l);
...
static b8 ST_lx_number(ST_lexer_t *l, ST_token_t *t) {
    ...
    if (c == '0' && (c2 == 'x' || c2 == 'X')) {
        ...
        ST_lx_advance_char(l);
        ST_lx_advance_char(l);
        while (ST_lx_peek_char(l) == '0' || ST_lx_peek_char(l) == '1' ||
               ST_lx_peek_char(l) == '_') {
            char d = ST_lx_advance_char(l);
            if (d != '_' && n < sizeof(buf) - 1)
                buf[n++] = d;
    ...
    if (ST_isalpha(ST_lx_peek_char(l))) {
        ST_lx_error(l, l->line, l->col, l->pos,
                    "invalid character in numeric literal");
        while (ST_isident(ST_lx_peek_char(l)))
            ST_lx_advance_char(l);
        return 0;
    }
...
static b8 ST_lx_ident(ST_lexer_t *l, ST_token_t *t) {
    ...
    if (ST_is_keyword(text))
        t->kind = ST_TKEYWORD;
    else if (ST_is_primitive(text))
        t->kind = ST_TTYPE;
    else
        t->kind = ST_TIDENT;
    t->line = line;
...
ST_tokens_t ST_lex(ST_arena_t *arena, ST_string_t src, ST_string_t file) {
    ...
    ST_token_t t;
    while (ST_lx_next(&l, &t))
        ST_da_append_arena(arena, &l.tokens, t);
...
void ST_dump_token(ST_tokens_t tokens) {
    ST_forrange(0, tokens.count) {
        ...
        printf(ST_sv_fmt ":%u:%u: %-14.*s " ST_sv_fmt, ST_sv_args(t->file),
               t->line, t->col, ST_sv_args(ST_token_kind_to_string(t->kind)),
               ST_sv_args(t->text));
        switch (t->kind) {
        case ST_TINT:
            printf("   (= %lld)", (long long)t->val.i);
            break;
        case ST_TFLOAT:
            printf("   (= %g)", t->val.f);
            break;
        ...
        case ST_TCHAR:
            ...
            break;
        case ST_TIDENT:
            printf("   (str \"" ST_sv_fmt "\")", ST_sv_args(t->text));
            break;
        case ST_TTYPE:
        case ST_TKEYWORD:
        case ST_TSYMBOL:
