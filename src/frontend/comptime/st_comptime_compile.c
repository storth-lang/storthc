...
static void ST_ct_cfail(ST_ct_compiler_t *cc, u32 line, u32 col, const char *fmt, ...) {
    if (cc->failed) // keep the first failure -- it's usually the most useful one
        return;
    cc->failed = 1;
    cc->err_line = line;
    cc->err_col = col;
...
static u32 ST_ct_declare_local(ST_ct_compiler_t *cc, ST_string_t name) {
    ...
    // same discipline a real stack-slot-based bytecode compiler uses.
...
// ---------------------------------------------------------------------
// Expressions -- each of these pushes exactly one value.
// ---------------------------------------------------------------------
...
static void ST_ct_compile_expr(ST_ct_compiler_t *cc, ST_expr_t *e) {
    ...
    switch (e->kind) {
        case ST_EX_INT:
        case ST_EX_CHAR:  // char/bool literals reuse 'ival' -- there's no
        ...
        case ST_EX_COMP_ERROR: {
            ...
            // COMP_ERROR halts the VM outright, but this expression is
            // still nominally "used" as a value by whatever's compiling it
            // (e.g. as an #if arm's condition, or a bare expression
            // statement) -- push nil so a caller still gets a balanced
            // stack shape if execution somehow continues past it.
            ST_ct_emit_op(cc->chunk, ST_OP_NIL, e->line);
            return;
        }
