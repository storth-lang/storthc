...
static i32 ST_fasm_slot(ST_ir_inst_t *v) {
    while (v->repl)
        v = v->repl;
    return -8 * (i32)(v->id + 1);
}

static void ST_fasm_int_narrow(FILE *out, ST_ty_t *ty) {
...
static void ST_fasm_load(FILE *out, const char *reg, ST_ir_inst_t *v) {
    fprintf(out, "    mov %s, [rbp%+d]\n", reg, ST_fasm_slot(v));
}

static void ST_fasm_fload(FILE *out, const char *reg, ST_ir_inst_t *v) {
    fprintf(out, "    movsd %s, [rbp%+d]\n", reg, ST_fasm_slot(v));
}

static u32 ST_fasm_tys_total_eightbytes(ST_tys_t *tys) {
    u32 total = 0;
    for (u32 i = 0; i < tys->count; i++) {
        ST_ty_t *t = tys->items[i];
        total += t ? (t->size + 7u) / 8u : 1;
    }
    return total;
}

static u32 ST_fasm_call_ret_count(ST_ir_inst_t *call_inst) {
    if (!call_inst->call.callee || !call_inst->call.callee->ty)
        return 1;
    u32 total_eb = ST_fasm_tys_total_eightbytes(&call_inst->call.callee->ty->rets);
    return total_eb ? total_eb : 1;
}

static i32 ST_fasm_ret_buf_off(ST_ir_inst_t *call_inst, u32 index) {
    return (i32)(8 * index) - (i32)call_inst->call.ret_buf_offset;
}

static void ST_fasm_mem_load(FILE *out, ST_ty_t *ty) {
    u32 size = ty && ty->size ? ty->size : 8;
    b8 _signed = ty && ty->kind == ST_TY_INT && ty->is_signed;
    switch (size) {
        case 1:
...
static void ST_fasm_mem_store(FILE *out, ST_ty_t *ty) {
    u32 size = ty && ty->size ? ty->size : 8;
    switch (size) {
        case 1:
...
static void ST_fasm_gen_scalar_data(FILE *out, u32 size, b8 is_float, i64 iv, f64 fv) {
    if (is_float) {
        if (size == 4) {
            u32 bits;
            float f = (float)fv;
...
static void ST_fasm_gen_struct_global_data(FILE *out, ST_ir_global_var_t *g) {
...
static void ST_fasm_generate_globals(FILE *out, ST_ir_module_t *m) {
    ...
    if (any_init) {
        ...
        ST_forrange(0, m->globals.count) {
            ...
            if (g->is_pub)
                fprintf(out, "public " ST_sv_fmt "\n", ST_sv_args(g->name));
            fprintf(out, "align %u\n", align);
            fprintf(out, ST_sv_fmt ":\n", ST_sv_args(g->name));
            if (g->field_inits.count) {
                ST_fasm_gen_struct_global_data(out, g);
                continue;
            }
            u32 size = g->ty && g->ty->size ? g->ty->size : 8;
            ST_fasm_gen_scalar_data(out, size, g->init_is_float, g->init_int, g->init_float);
        }
    }
    if (any_uninit) {
...
static void ST_fasm_generate_strs(FILE *out, ST_ir_module_t *m) {
    if (!m->strs.count)
        return;
    fprintf(out, "\nsection '.rodata'\n");
...
static u32 ST_fasm_emit_call_args(FILE *out, ST_ir_insts_t *args, u32 reserved_int_regs,
    ...
    ST_forrange(0, n) {
        ...
        ST_ir_inst_t *arg = args->items[i];
        if (arg->ty && ST_ty_is_float(arg->ty)) {
            ST_fasm_fload(out, xmm_regs[float_idx], arg);
            if (arg->ty->size == 4)
                fprintf(out, "    cvtsd2ss %s, %s\n", xmm_regs[float_idx], xmm_regs[float_idx]);
            float_idx++;
        } else {
            ST_fasm_load(out, arg_regs[int_idx], arg);
            int_idx++;
        }
    }
...
static void ST_fasm_truncate_int(FILE *out, ST_ty_t *ty) {
...
static void ST_fasm_icmp(FILE *out, ST_ir_inst_t *in, const char *setcc) {
    ST_fasm_load(out, "rax", in->bin.l);
    ST_fasm_load(out, "rcx", in->bin.r);
    fprintf(out, "    cmp rax, rcx\n");
    fprintf(out, "    %s al\n", setcc);
    fprintf(out, "    movzx rax, al\n");
}

static void ST_fasm_fcmp(FILE *out, ST_ir_inst_t *in, ST_ir_op_t kind) {
    ST_fasm_fload(out, "xmm0", in->bin.l);
    ST_fasm_fload(out, "xmm1", in->bin.r);
...
static void ST_fasm_generate_inst(FILE *out, ST_fasm_ctx_t *ctx, ST_ir_inst_t *in) {
    ...
    switch (in->kind) {
        ...
        case ST_IR_ADD:
            ...
            ST_fasm_truncate_int(out, in->ty);
            break;
        case ST_IR_SUB:
            ST_fasm_load(out, "rax", in->bin.l);
            ST_fasm_load(out, "rcx", in->bin.r);
        ...
        case ST_IR_FDIV:
            ...
            break;
        case ST_IR_NEG:
            ST_fasm_load(out, "rax", in->unary.v);
            fprintf(out, "    neg rax\n");
            ST_fasm_truncate_int(out, in->ty);
            break;
        case ST_IR_FNEG:
            ST_fasm_fload(out, "xmm0", in->unary.v);
        ...
        case ST_IR_NOT:
            ...
            break;
        case ST_IR_ICMP_EQ:
            ST_fasm_icmp(out, in, "sete");
            break;
        case ST_IR_ICMP_NE:
            ST_fasm_icmp(out, in, "setne");
            break;
        case ST_IR_ICMP_SLT:
            ST_fasm_icmp(out, in, "setl");
            break;
        case ST_IR_ICMP_SLE:
            ST_fasm_icmp(out, in, "setle");
            break;
        case ST_IR_ICMP_SGT:
            ST_fasm_icmp(out, in, "setg");
            break;
        case ST_IR_ICMP_SGE:
            ST_fasm_icmp(out, in, "setge");
            break;
        case ST_IR_ICMP_ULT:
            ST_fasm_icmp(out, in, "setb");
            break;
        case ST_IR_ICMP_ULE:
            ST_fasm_icmp(out, in, "setbe");
            break;
        case ST_IR_ICMP_UGT:
            ST_fasm_icmp(out, in, "seta");
            break;
        case ST_IR_ICMP_UGE:
            ST_fasm_icmp(out, in, "setae");
            break;
        case ST_IR_FCMP_EQ:
            ST_fasm_fcmp(out, in, ST_IR_FCMP_EQ);
            break;
        case ST_IR_FCMP_NE:
            ST_fasm_fcmp(out, in, ST_IR_FCMP_NE);
            break;
        case ST_IR_FCMP_LT:
            ST_fasm_fcmp(out, in, ST_IR_FCMP_LT);
            break;
        case ST_IR_FCMP_LE:
            ST_fasm_fcmp(out, in, ST_IR_FCMP_LE);
            break;
        case ST_IR_FCMP_GT:
            ST_fasm_fcmp(out, in, ST_IR_FCMP_GT);
            break;
        case ST_IR_FCMP_GE:
            ST_fasm_fcmp(out, in, ST_IR_FCMP_GE);
            break;
        case ST_IR_CAST: {
            ...
            if (sf && df) {
                ...
                ST_fasm_int_narrow(out, dty);
            } else if (!sf && df) {
                ST_fasm_load(out, "rax", src);
        ...
        case ST_IR_CALL: {
            u32 rc = ST_fasm_call_ret_count(in);
            u32 reserved = 0;
            if (rc > 2) {
                fprintf(out, "    lea rdi, [rbp%+d]\n", ST_fasm_ret_buf_off(in, 0));
                reserved = 1;
            }
            u32 stack_bytes = 0;
            u32 float_idx = ST_fasm_emit_call_args(out, &in->call.args, reserved, &stack_bytes);
        ...
        case ST_IR_CALL_INDIRECT: {
            u32 stack_bytes = 0;
            u32 float_idx = ST_fasm_emit_call_args(out, &in->call_ind.args, 0, &stack_bytes);
            fprintf(out, "    mov al, %u\n", float_idx);
            ST_fasm_load(out, "r10", in->call_ind.callee_ptr);
            fputs("    call r10\n", out);
            if (stack_bytes)
                fprintf(out, "    add rsp, %u\n", stack_bytes);
        ...
        case ST_IR_ALLOCA:
            ...
            break;
        case ST_IR_LOAD:
            ST_fasm_load(out, "rcx", in->load.addr);
            if (in->ty && ST_ty_is_float(in->ty)) {
                if (in->ty->size == 4) {
                ...
                ST_fasm_mem_load(out, in->ty);
            break;
        case ST_IR_STORE:
            if (in->ty && ST_ty_is_float(in->ty)) {
                ST_fasm_fload(out, "xmm0", in->store.v);
                ST_fasm_load(out, "rcx", in->store.addr);
                if (in->ty->size == 4) {
                    fprintf(out, "    cvtsd2ss xmm0, xmm0\n");
                ...
                ST_fasm_load(out, "rax", in->store.v);
                ST_fasm_load(out, "rcx", in->store.addr);
                if (in->store.addr->kind == ST_IR_ALLOCA && in->ty && in->ty->size &&
                    in->ty->size < 8) {
                    ST_fasm_int_narrow(out, in->ty);
                    fprintf(out, "    mov [rcx], rax\n");
                } else {
                    ST_fasm_mem_store(out, in->ty);
                }
            }
            break;
        case ST_IR_ADDR:
            ST_fasm_load(out, "rax", in->addr.base);
            if (in->addr.index) {
                ST_fasm_load(out, "rcx", in->addr.index);
...
static ST_ir_inst_t *ST_fasm_phi_incoming(ST_ir_inst_t *in, ST_ir_block_t *from) {
...
static void ST_fasm_generate_phi_copies(FILE *out, ST_ir_block_t *from, ST_ir_block_t *to) {
    for (ST_ir_inst_t *inst = to->first; inst; inst = inst->next) {
        if (inst->removed || inst->kind != ST_IR_PHI)
            continue;
        ST_ir_inst_t *v = ST_fasm_phi_incoming(inst, from);
        if (!v)
            continue;
        ST_fasm_load(out, "rax", v);
        fprintf(out, "push rax\n");
    }
    for (ST_ir_inst_t *inst = to->last; inst; inst = inst->prev) {
        if (inst->removed || inst->kind != ST_IR_PHI)
            continue;
        if (!ST_fasm_phi_incoming(inst, from))
            continue;
        fprintf(out, "pop rax\n");
        fprintf(out, "mov [rbp%+d], rax\n", ST_fasm_slot(inst));
    }
...
static b8 ST_fasm_edge_has_phi(ST_ir_block_t *to) {
    for (ST_ir_inst_t *inst = to->first; inst; inst = inst->next)
        if (!inst->removed && inst->kind == ST_IR_PHI)
            return 1;
    return 0;
...
static void ST_fasm_generate_term(FILE *out, ST_fasm_ctx_t *ctx, ST_ir_block_t *b) {
    ...
    switch (t->kind) {
        case ST_IR_TERM_RET: {
            if (t->rets.count > 2) {
                ...
                ST_forrange(0, t->rets.count) {
                    ST_fasm_load(out, "rax", t->rets.items[i]);
                ...
                if (t->rets.count >= 1) {
                    if (t->rets.items[0]->ty && ST_ty_is_float(t->rets.items[0]->ty)) {
                        ST_fasm_fload(out, "xmm0", t->rets.items[0]);
                        if (t->rets.items[0]->ty->size == 4)
                            fprintf(out, "    cvtsd2ss xmm0, xmm0\n");
                    } else
                        ST_fasm_load(out, "rax", t->rets.items[0]);
                }
                if (t->rets.count >= 2) {
                    if (t->rets.items[1]->ty && ST_ty_is_float(t->rets.items[1]->ty)) {
                        ST_fasm_fload(out, "xmm1", t->rets.items[1]);
                        if (t->rets.items[1]->ty->size == 4)
                            fprintf(out, "    cvtsd2ss xmm1, xmm1\n");
                    } else
                        ST_fasm_load(out, "rdx", t->rets.items[1]);
                }
            }
            fprintf(out, "    leave\n");
            fprintf(out, "    ret\n");
        ...
        case ST_IR_TERM_BR:
            ST_fasm_generate_phi_copies(out, b, t->t_block);
        ...
        case ST_IR_TERM_COND_BR: {
            ...
            fprintf(out, "    test rax, rax\n");
            if (ST_fasm_edge_has_phi(t->f_block)) {
                fprintf(out, "    jz .bb%u_edge_f\n", b->id);
                ST_fasm_generate_phi_copies(out, b, t->t_block);
                fprintf(out, "    jmp .bb%u\n", t->t_block->id);
                fprintf(out, "    .bb%u_edge_f:\n", b->id);
                ST_fasm_generate_phi_copies(out, b, t->f_block);
                fprintf(out, "    jmp .bb%u\n", t->f_block->id);
            } else {
                fprintf(out, "    jz .bb%u\n", t->f_block->id);
                ST_fasm_generate_phi_copies(out, b, t->t_block);
...
static u32 ST_fasm_layout_fn(ST_ir_fn_t *fn, ST_fasm_ctx_t *ctx) {
    u32 cur = fn->next_value_id * 8u;

    ctx->fn = fn;
    ctx->hidden_ret_off = 0;
    if (fn->ty && ST_fasm_tys_total_eightbytes(&fn->ty->rets) > 2) {
        cur += 8;
...
static void ST_fasm_generate_fn(FILE *out, ST_ir_fn_t *fn) {
    ST_fasm_ctx_t ctx;
    u32 extra = ST_fasm_layout_fn(fn, &ctx);
    ctx.next_int_arg = 0;
    ...
    ST_forrange(0, fn->blocks.count) {
        ...
        fprintf(out, "    .bb%u:\n", b->id);
        for (ST_ir_inst_t *in = b->first; in; in = in->next)
            ST_fasm_generate_inst(out, &ctx, in);
        ST_fasm_generate_term(out, &ctx, b);
    }
}
...
b8 ST_fasm_generate(FILE *out, ST_ir_module_t *m, ST_string_t src, ST_string_t file,
    ...
    ST_forrange(0, m->fns.count) {
        ...
        if (fn->is_pub)
            fprintf(out, "public " ST_sv_fmt "\n", ST_sv_args(fn->name));
        ST_fasm_generate_fn(out, fn);
    }

    if (emit_entry) {
        ST_ir_fn_t *main_fn = ST_ir_module_find_fn(m, ST_cstr_to_str("main"));
        b8 main_wants_args = main_fn && main_fn->ty && main_fn->ty->params.count == 1;

        fprintf(out, "\npublic _start\n");
        fprintf(out, "\n_start:\n");
        if (main_wants_args) {
            fprintf(out, "      mov r12, [rsp]\n");
            fprintf(out, "      lea r13, [rsp+8]\n");
            fprintf(out, "      mov rax, r12\n");
            fprintf(out, "      imul rax, 16\n");
            fprintf(out, "      sub rsp, rax\n");
            fprintf(out, "      mov r14, rsp\n");
            fprintf(out, "      xor r15, r15\n");
            fprintf(out, "    .argv_loop:\n");
            fprintf(out, "      cmp r15, r12\n");
            fprintf(out, "      jge .argv_done\n");
            fprintf(out, "      mov rbx, [r13 + r15*8]\n");
            fprintf(out, "      mov rax, rbx\n");
            fprintf(out, "      xor rcx, rcx\n");
            fprintf(out, "    .argv_strlen:\n");
            fprintf(out, "      cmp byte [rax], 0\n");
            fprintf(out, "      je .argv_strlen_done\n");
            fprintf(out, "      inc rax\n");
            fprintf(out, "      inc rcx\n");
            fprintf(out, "      jmp .argv_strlen\n");
            fprintf(out, "    .argv_strlen_done:\n");
            fprintf(out, "      mov rax, r15\n");
            fprintf(out, "      imul rax, 16\n");
            fprintf(out, "      mov [r14 + rax], rbx\n");
            fprintf(out, "      mov [r14 + rax + 8], rcx\n");
            fprintf(out, "      inc r15\n");
            fprintf(out, "      jmp .argv_loop\n");
            fprintf(out, "    .argv_done:\n");
            fprintf(out, "      mov rdi, r14\n");
            fprintf(out, "      mov rsi, r12\n");
        }
